...
function readFileList (dir, src, pages, langObj, filesList = []) {
  ...
  files.forEach((item, index) => {
    ...
    if (stat.isDirectory()) {
      readFileList(path.join(dir, item), src, pages, langObj, filesList)// 递归读取文件
    } else {
      let ContentLine = ''
      let fileContent = ''
      let hasReplaced  = false
      let path2 = fullPath.replace(path.join(__dirname, dirU + 'src' + dirU), '')
      if (pages.some(pagedir => path2.includes(pagedir))) {
        let newPath = path.join(__dirname, dirU + src + dirU + path2)
...
module.exports.writeLang = (src = 'srcDist', pages = 'pages', LangJson = '/zh_cn.json') => {
  let jsonFile = fs.readFileSync(path.join(__dirname, LangJson), 'utf-8')
  ...
  delDir(path.join(__dirname, dirU + src))
  readFileList(filePath, src, pages, langObj)
}
