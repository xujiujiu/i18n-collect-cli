...
let lang = JSON.parse(fs.readFileSync(path.join(__dirname, '/lang.json'), 'utf-8'))
let langObj = {}
  ...
  files.forEach((item, index) => {
    ...
    if (stat.isDirectory()) {
      ...
      if (path2.includes('pages') || path2.includes('components')) {
        let newPath = path.join(__dirname, dirU + 'srcDist' + dirU + path2)
...
delDir(path.join(__dirname, dirU + 'srcDist'))
